...
export default function ProductOverview() {
  ...
  useEffect(() => {
    if (status == "loading") {
      axios
        .get(import.meta.env.VITE_BACKEND_URL + "/products/" + params.productID) //Backend eke haduwa getProductById eka call wenne
        .then((response) => {
  ...
  return (
    <>
      {status == "loading" && (
        <h1 className="text-2xl font-medium tracking-widest text-gray-700 flex items-center ">
          Loading...
        </h1>
      )}
      {status == "error" && (
        <h1 className="text-2xl font-medium tracking-widest text-gray-700">
      ...
      {status == "success" && (
        <div className="w-full min-h-[calc(100vh-100px)] flex flex-col lg:flex-row ">
          ...
          <div className="w-full lg:w-1/2 h-auto lg:h-[calc(100vh-100px)] bg-white flex justify-center items-start lg:items-center overflow-auto">
            <div className="w-full lg:w-[85%] max-h-[90%] bg-gray-50 rounded-2xl shadow-xl p-6 lg:p-8 flex flex-col gap-5">
              {/* Product Header */}
              <div>
